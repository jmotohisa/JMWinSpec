...
-------------------------------------------------------------------------------
  ...
  */
  double  offset_x                ;//      3000  offset for absolute data scaling
  double  factor_x                ;//      3008  factor for absolute data scaling
  char    current_unit_x          ;//      3016  selected scaling unit           
  char    reserved1_x             ;//      3017  reserved                        
  char    string_x[40]            ;//      3018  special string for scaling      
  char    reserved2_x[40]         ;//      3058  reserved                        
  char    calib_valid_x           ;//      3098  flag if calibration is valid    
  char    input_unit_x            ;//      3099  current input units for         
                                 //            "calib_value"                  
  char    polynom_unit_x          ;//      3100  linear UNIT and used            
                                 //            in the "polynom_coeff"         
  char    polynom_order_x         ;//      3101  ORDER of calibration POLYNOM    
  char    calib_count_x           ;//      3102  valid calibration data pairs    
  double  pixel_position_x[10]    ;//      3103  pixel pos. of calibration data  
  double  calib_value_x[10]       ;//      3183  calibration VALUE at above pos  
  double  polynom_coeff_x[6]      ;//      3263  polynom COEFFICIENTS            
  double  laser_position_x        ;//      3311  laser wavenumber for relativ WN 
  char    reserved3_x             ;//      3319  reserved                        
  BYTE    new_calib_flag_x        ;//      3320  If set to 200, valid label below
  char    calib_label_x[81]       ;//      3321  Calibration label (NULL term'd) 
  char    expansion_x[87]         ;//      3402  Calibration Expansion area      
  /*
-------------------------------------------------------------------------------

                        START OF Y CALIBRATION STRUCTURE (3489 - 3977)
  */
  double  offset_y                ;//      3489  offset for absolute data scaling
  double  factor_y                ;//      3497  factor for absolute data scaling
  char    current_unit_y          ;//      3505  selected scaling unit           
  char    reserved1_y             ;//      3506  reserved                        
  char    string_y[40]            ;//      3507  special string for scaling      
  char    reserved2_y[40]         ;//      3547  reserved                        
  char    calib_valid_y           ;//      3587  flag if calibration is valid    
  char    input_unit_y            ;//      3588  current input units for         
                                 //            "calib_value"                   
  char    polynom_unit_y          ;//      3589  linear UNIT and used            
                                 //            in the "polynom_coeff"          
  char    polynom_order_y         ;//      3590  ORDER of calibration POLYNOM    
  char    calib_count_y           ;//      3591  valid calibration data pairs    
  double  pixel_position_y[10]    ;//      3592  pixel pos. of calibration data  
  double  calib_value_y[10]       ;//      3672  calibration VALUE at above pos  
  double  polynom_coeff_y[6]      ;//      3752  polynom COEFFICIENTS            
  double  laser_position_y        ;//      3800  laser wavenumber for relativ WN 
  char    reserved3_y             ;//      3808  reserved                        
  BYTE    new_calib_flag_y        ;//      3809  If set to 200, valid label below
  char    calib_label_y[81]       ;//      3810  Calibration label (NULL term'd) 
  char    expansion_y[87]         ;//      3891  Calibration Expansion area      
